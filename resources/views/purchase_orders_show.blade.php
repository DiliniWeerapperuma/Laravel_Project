...
<body>
    <div class="container">
        <center>
            <h3><p class="text-danger">VIEW PURCHASE ORDER</p></h3>
        </center>

        <div class="row">
            <div class="col-md-6 detail-group">
                <label for="zone">Zone</label>
                <div id="zone">{{ $zone->zone_code }}</div>
                <input type="hidden" id="zone_id" name="zone_id" value="{{ $zone->id }}">
            </div>

            <div class="col-md-6 detail-group">
                <label for="region">Region</label>
                <div id="region">{{ $region->region_code }}</div>
        ...
        <div class="row">
            <div class="col-md-6 detail-group">
                ...
                <div id="territory">{{ $territory->territory_code }}</div>
                <input type="hidden" id="territory_id" name="territory_id" value="{{ $territory->id }}">
            </div>

            <div class="col-md-6 detail-group">
                <label for="distributor">Distributor</label>
                <div id="distributor">{{ $distributor->name }}</div>
                <input type="hidden" id="distributor_id" name="distributor_id" value="{{ $distributor->id }}">
            </div>

            <div class="col-md-6 detail-group">
                <label for="date">Today's Date</label>
                <div id="date">{{ $purchase_order->date }}</div>
                <input type="hidden" id="date_value" name="date_value" value="{{ $purchase_order->date }}">
            </div>

            <div class="col-md-6 detail-group">
                <label for="po_no">PO No</label>
                <div id="po_number">{{ $purchase_order->po_number }}</div>
                <input type="hidden" id="po_num_value" name="po_num_value" value="{{ $purchase_order->po_number }}">
            </div>
        ...
        <form id="purchaseOrderForm">
            <table class="table table-striped mt-5">
                ...
                <tbody>
                    ...
                    <tr>
                        <td>
                            <input type="text" class="form-control" name="sku_code[]" value="{{ $datas->sku_code }}" readonly>
                        </td>
                        <td>
                            <input type="text" class="form-control" name="sku_name[]" value="{{ $datas->sku_name }}" readonly>
                        </td>
                        <td>
                            <input type="text" class="form-control unit-price" name="unit_price[]" value="{{ $datas->price }}" readonly>
                        </td>
                        {{-- <td>
                            <input type="text" class="form-control" name="available_quantity[]" value="{{ $datas->qty }}" readonly>
                        </td> --}}
                        <td>
                            <input type="number" class="form-control enter-quantity" name="enter_quantity[]" value="{{ $datas->qty }}" oninput="calculateTotalPrice(this)" readonly>
                        </td>
                        <td>
                            <input type="text" class="form-control total-price" name="total_price[]" value="{{ $datas->total_amount }}" readonly>
                        </td>
                        <td>
                            <input type="hidden" name="product_id[]" value="{{ $datas->id }}" readonly>
                        </td>
                    </tr>
                    @endforeach
                    ...
                    <tr>
                        ...
                        {{-- <td></td> --}}
                        <td>
                            <input type="text" class="form-control total-price" name="total_price[]" value="{{$purchase_order->total_amount}}" readonly>
                        </td>
